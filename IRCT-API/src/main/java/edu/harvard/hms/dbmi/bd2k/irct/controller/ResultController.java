...
//		EntityManager entityManager = objectEntityManager.createEntityManager();
	...
	public ResultDataStream getResultDataStream(User user, Long resultId,
		...
		if (results == null || results.size() == 0) {
			...
			logger.trace("getResultDataStream() there are ```"+results.size()+"``` results found.");
		}
		Result result = results.get(0);
		
		logger.trace("getResultDataStream() The first result status is "+result.getResultStatus().name());
		...
		logger.trace("getResultDataStream() ResultDataConverter has been retrieved");
		if (rdc == null) {
