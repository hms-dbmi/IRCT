...
public class GLM {
	...
	public static void main(String[] args) {
		...
		SciDBArray arrayB = new SciDBArray("response");
		...
		SciDBArray arrayC = new SciDBArray("weights");
		arrayC.addAttribute("attrB1", SciDBDataType.DOUBLE);
		arrayC.addDimension("i", new SciDBDimension(0, 3, 4, 0));
		arrayC.addDimension("j", new SciDBDimension(0, 3, 4, 0));
		SciDBOperation arrayCCreateOperation = sciDB.create(arrayC);
		System.out.println(arrayCCreateOperation.toAFLQueryString());

		
		

		SciDBOperation glmOperation = sciDB.glm(arrayA, arrayB, arrayC,"gamma", "log");
		
		System.out.println(glmOperation.toAFLQueryString());
		// sciDB.close();
	}
